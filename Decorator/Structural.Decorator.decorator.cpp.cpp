...
// We want to add to the shapes. Dynamic decorator: Cause we make it in run time.

struct ColoredShape : Shape
{
  // Reference to the shape which we are decorating.
  // The down side is that we do not access the underlying
  // API of the shape object. If it is a circle for example,
  // we cannot call resize on a ColoredShape object.
  Shape& shape;
  string color;
...
// Inherit from the template argument.
template <typename T> struct ColoredShape2 : T
{
  ...
  // To support TransparentShape2<ColoredShaped2><Square>> sq{10, 'red', 44}
